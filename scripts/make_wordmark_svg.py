"""
Render "PIO" as an EXTRUDED 3D wordmark rasterized to ASCII, and emit it as an
...
TEXT = os.environ.get("WORDMARK_TEXT", "PIO")
...
FOCAL = 4.15
FIT = 0.80             # fraction of the grid the widest pose may use
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">Pio@github: ~$ ./wordmark.sh --3d</text>')
